...
namespace AmiumScripter.Controls
{
    public abstract class BaseControl : Control
    {
        ...
        {
         ControlManager.Register(signal.Name, this);
    ...
    public class SignalView : BaseControl
    {
        ...
        public override void Update()
        {
           // Signal source = SignalPool.Get<Signal>(sourceName);
            SignalText = _signal.Text;
            SignalUnit = _signal.Unit;
            if (_signal.Value is IFormattable formattable)
            {
                SignalValue = formattable.ToString(_signal.Format, CultureInfo.InvariantCulture);
            }
            else
            {
                SignalValue = _signal.Value?.ToString() ?? string.Empty;
            }
